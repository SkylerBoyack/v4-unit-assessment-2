...
compareNums = function(num, num2){
    if(num > num2){return num
    }else if(num === num2){ return num}
}
...
for (const odds in shapes){
    if(shapes.value(odds)){
        if((shapes[odds] % 2) !=0){
            delete shapes
        }
    }
...
// const oddRemover = function(shapes){
// for (const i = 0; i < shapes.length; i++) {
//     if ((shapes[i] % 2) != 0) { 
//         delete shapes[i]
//     }
//   }
...
*/
 ...
 this.tricks = ['sit','shake']
const fido = new Dog("Fido",3,"Jack Russell",tricks) 
...
function dogIntro(treat,toy){
 return `NAME is a BREED that loves ${this.treat} and their ${this.toy}! `
...
//CODE HERE
  let phone1 = new Phone('apple','lame',128,"blue",false)
  
  let phone2 = new Phone('android','big',100,"black",false)
  
  let phone3 = new Phone('nokia','why',5,"red",false)
